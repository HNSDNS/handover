...
class Ethereum {
  ...
  async dnsRecord(name, type, node, resolver) {
  ...
  async resolveDnsWithResolver(name, type, node, resolver) {
    let record = await this.dnsRecord(name, type, node, resolver);
    if (record) {
      if (type === wire.types.NS) {
        let dsSet = await this.dnsRecord(name, wire.types.DS, node, resolver);
        if (dsSet)
          return Buffer.concat([record, dsSet]);
      }
    ...
    let nsSet = await this.dnsRecord(sname, wire.types.NS, node, resolver);
    if (nsSet) {
      let dsSet = await this.dnsRecord(sname, wire.types.DS, node, resolver);
      if (dsSet)
    ...
    return await this.dnsRecord(sname,
      wire.types.CNAME,
      node,
      resolver
    );
