...
class Ethereum {
  ...
  async resolveDnsWithResolver(name, type, node, resolver) {
    ...
    if (record && record !== '0x') {
      ...
      const res = Buffer.from(record.substr(2), 'hex');

      // if it's a referral add DS
      if (type === wire.types.NS) {
        const ds = await this.resolveDnsWithResolver(name,
          wire.types.DS,
          node,
          resolver
        );
    ...
    const sname = this.toNode(name) + ".";
    let ns = await this.resolveDnsWithResolver(sname,
      wire.types.NS,
      node,
      resolver
    );
    ...
    if (ns) {
      let ds = await this.resolveDnsWithResolver(sname,
        wire.types.DS,
        node,
        resolver
      );
      ...
      if (ds)
        return Buffer.concat([ns, ds]);

      return ns;
    }

    return await this.resolveDnsWithResolver(sname,
      wire.types.CNAME,
      node,
      resolver
    );
